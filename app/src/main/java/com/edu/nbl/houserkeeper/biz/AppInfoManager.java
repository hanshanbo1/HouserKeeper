...
public class AppInfoManager {
    ...
    public void killALLProcesses() {
        ...
        for (RunningAppProcessInfo appProcessInfo : appProcessInfos) {//遍历所有进程
            if (appProcessInfo.importance >= RunningAppProcessInfo.IMPORTANCE_SERVICE) {
                String packageName = appProcessInfo.processName;//保留服务进程  后台进程  空进程
                try {
    ...
    public static final int RUNING_APP_TYPE_SYS = 2;//2表示系统进程
    public static final int RUNING_APP_TYPE_USER = 0;//0表示用户进程
        ...
        for (RunningAppProcessInfo appProcessInfo : appProcessInfos) {//遍历所有的进程
            ...
            if (importance >= RunningAppProcessInfo.IMPORTANCE_SERVICE) {//服务进程>后台进程>空进程
                ...
                try {
                    ...
                    // TODO: 2017/8/14 手机加速页面要更改的代码
                    RunningAppInfo runingAppInfo = new RunningAppInfo(false, icon, packageName, lableName, false, size);
                    // 判断是否是系统进程
                    if ((applicationInfo.flags & ApplicationInfo.FLAG_SYSTEM) != 0) {//是系统进程
