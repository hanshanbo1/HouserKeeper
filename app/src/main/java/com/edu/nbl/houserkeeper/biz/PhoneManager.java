...
//		am = (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE);
	...
	public String getPhoneCpuName() {
		try {
			...
			BufferedReader br = new BufferedReader(fr);
			String text = br.readLine();
			String[] array = text.split(":\\s+",2);//[processor	,0]
			return array[1];
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		} catch (IOException e) {
	...
	public int getPhoneCpuNumber() {
		class CpuFilter implements FileFilter {
			public boolean accept(File file) {
				if (Pattern.matches("cpu[0-9]",file.getName())) {//cpu0 cpu1 cpu2...cpu9
					return true;
				}
				return false;
			}
		...
		try {
			...
			File[] files = dir.listFiles(new CpuFilter());//files=[cpu0]
			return files.length;
		} catch (Exception e) {
			e.printStackTrace();
			return 1;
	...
	public String getMaxPhotoSize() {
		...
		for (Size s : sizes) {
			if (size == null) {
				size = s;
			} else if (size.height * size.width < s.height * s.width) {//用后面的图片和前面的图片比
				//找最大的图片
				size = s;
			}
		}
