...
public class FileTypeUtil {
     ...
     *             通过文件名称 获取文件图标以及类型
     */
    public static String[] getFileIconAndTypeName(File file) {//例如 a.txt
        // 定义数组保存文件类型以及图标 iconAndTypes[0] 表示文件图标 iconAndTypes[1] 文件类型
        String[] iconAndTypes = new String[2];
        // 首先设置默认文件图标以及类型
        iconAndTypes[0] = "icon_file";
        iconAndTypes[1] = "*/*";
        //iconAndTypes={"icon_file","*/*"}
        if (file.isFile()) {//例如：file=a.txt
            // 根据文件名称进行判断
            ...
            //获取文件后缀名称
            String end = name.substring(dotIndex + 1, name.length())//[2,5）  txt
                    .toLowerCase();//txt
            if (end.equals("")) {
                return iconAndTypes;
            }

            // 对  ICON_TYPE_Table进行遍历   找到对应图标以及类型
            for (int i = 0; i < ICON_TYPE_Table.length; i++) {//遍历二维数组
//end.equals(ICON_TYPE_Table[i][0])和所有的后缀名比较
                if (end.equals(ICON_TYPE_Table[i][0])) {//txt.equals()
                    //{ "txt", "icon_text_plain", TYPE_TXT }  123行
                    iconAndTypes[0] = ICON_TYPE_Table[i][1];//123
                    iconAndTypes[1] = ICON_TYPE_Table[i][2];
                }
            }
            return iconAndTypes;// iconAndTypes={"icon_text_plain","txt"};
        }
        return iconAndTypes;
    }
